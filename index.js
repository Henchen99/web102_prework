...
const formattedBackers = totalBackers.toLocaleString(); 

contributionsCard.textContent = formattedBackers;
...
raisedCard.textContent = `$${formattedRaised}`;
...
const totalUnfunded = GAMES_JSON.reduce((acc, games) => {
    return acc + (games.goal > games.pledged ? 1 : 0);
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const unfundedGamesExplanation = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames}. Currently, ${totalUnfunded}  game${totalUnfunded === 1 ? '' : 's'} remain unfunded. We need your help funding these games!`;

// create a new DOM element containing the template string and append it to the description container

const newParagraph = document.createElement('p');
newParagraph.innerHTML = unfundedGamesExplanation;
